...
| `suggest [--doc path] [--wikilink]` | 안 적힌 연결의 휴리스틱 후보: 공유 희귀 term, 공유 요구사항 ID, 같은 코드 파일 참조. 0.2초, LLM 없음. `--wikilink`는 바로 붙여넣을 `[[링크]]`를 출력 — 진짜 연결은 문서 본문으로 승격하세요, 그곳이 모든 도구가 읽는 자리입니다 |
...
2. 어시스턴트는 방금 바뀐 문서의 상위 후보만 읽고 진짜 연결을 `edge add`로 확정합니다. 비용은 수정량에 비례하고, 코퍼스 크기에 비례하지 않습니다.
