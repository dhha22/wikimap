...
### 블라인드 135질의 vs 신선 재빌드 그래프 — 사전 등록 판정 (1.1.0)

이전 비교에는 모두 찌를 수 있는 약점이 하나씩 있었습니다: 블라인드 20질의는 표본이 작고, graphify는 더 작은 옛 코퍼스에서 만든 그래프로 뛰었습니다. 이번 라운드는 그 두 가지 핑계를 모두 제거했고, 합격선을 **질의를 하나도 돌리기 전에 명문화**했습니다: recall@5에서 McNemar 정확 검정 p < 0.05 *그리고* 절대차 +0.15 이상일 때만 "압도"를 주장할 수 있다. 하나라도 놓치면 앞서 있어도 그 표현은 쓰지 않는다.

설정: 실사용 중인 한/영 vault의 622파일 스냅샷을 출제 전에 동결. **독립 에이전트 23명**이 코퍼스를 겹치지 않게 나눠 읽고 — 검색 도구 금지, 무엇을 테스트하는지 미고지 — 층화 질문을 출제했습니다(규칙·조건, 의도적 어휘 갭, 사실값, 결정 경위, 멀티홉, 그리고 원 질의와 쌍을 이루는 기계적 오타 변형). 전 질문이 기존 벤치마크 질의 431건 대비 유사도 게이트(< 0.60)와 문항당 독립 검증 에이전트를 통과한 뒤 135문항으로 동결. graphify는 **똑같은 스냅샷에서 자기 파이프라인으로 새 그래프**를 받았고(노드 2,426, md 418/418 커버 — 추출에 약 57분·약 3,500만 토큰), wikimap은 같은 스냅샷을 **1.42초·$0**에 인덱싱했습니다.

```mermaid
xychart-beta
    title "블라인드 135질의 — recall / MRR (높을수록 좋음)"
    x-axis ["recall@1", "recall@3", "recall@5", "recall@10", "MRR"]
    y-axis "score" 0 --> 1
...
<sub>막대 = wikimap 1.1.0 **단일 질의** · **3표현 fan-out** · 선 = graphify (신선 그래프, BFS)</sub>

| 지표 | wikimap — 단일 | wikimap — fan-out | graphify (신선) |
|---|---|---|---|
| recall@5 | **0.830** | 0.741 | 0.570 |
| MRR | **0.684** | 0.631 | 0.443 |
| 완전 미스 (rank 0) | 19 | **1** | 30 |
| evidence@10 (스니펫에 정답 줄) | 0.474 | **0.615** | 0.007* |
| 인덱스/그래프 빌드 | **1.42초, $0** | 동일 | 약 57분, 약 3,500만 토큰 |

<sub>*graphify는 원문 줄이 아닌 엔티티 label을 돌려주므로 성격이 다른 수치 — 공정 비교가 아니라 완전성을 위해 병기.</sub>

**판정: 기준 충족.** recall@5 차이 +0.259 (부트스트랩 95% CI [+0.163, +0.356]); 한쪽만 성공한 51문항 중 wikimap 43승, graphify 8승 (McNemar p < 0.00001). fan-out 모드도 독립적으로 사전 등록 기준을 넘습니다(+0.170, p = 0.0022).

헤드라인보다 중요한 정직한 발견 세 가지:

- **wikimap이 못 이기는 유일한 곳은 의도적 어휘 갭** — 문서의 어휘를 피해서 표현한 질문. 여기서는 graphify와 동률입니다(0.517 vs 0.483). 키워드 매칭의 구조적 한계이며, 이 55문항은 골든셋으로 동결되어 앞으로 모든 기능이 움직여야 할 과녁이 됐습니다.
- **fan-out이 정확히 그 질문들을 구합니다** (어휘 갭 recall 0.517 → 0.690, 완전 미스 19 → 1). 대신 쉬운 질의의 상위 순위가 희석됩니다. 1.2.0의 교훈: 항상이 아니라, 첫 검색이 약해 보일 때 *조건부로* fan-out.
- **오타 내성은 공짜로 얻어져 있었습니다**: 오타/원본 12쌍(공백 붕괴, 인접 자모 오타, IME 상태 오타)에서 wikimap의 손실은 0 — CJK 부분문자열 매칭은 사라진 공백을 신경 쓰지 않습니다.

### 조건부 fan-out — 벤치마크가 남긴 숙제의 이행 (1.2.0)

블라인드 벤치마크는 정확한 할 일 목록을 남겼고, 1.2.0은 그 목록을 그대로 실행한 릴리즈입니다. 두 변경 모두 미스 분류에서 나왔고, 어느 쪽도 문서 랭킹은 건드리지 않습니다(290 + 135 전체 벤치마크 랭킹에서 변화 0 — 늘 그렇듯 릴리즈마다 실측 검증).

**질의가 정말 필요로 할 때만 fan-out.** 기존 `weak` 플래그는 "재작성하라"는 신호여야 했지만 자연어 질의 135건 중 135건에서 켜졌습니다 — partial/OR 모드를 약함으로 취급했는데, 대화형 질문은 설계상 전부 그 모드로 돌기 때문입니다. 재설계된 신호는 **죽은 어휘**에서 발동합니다: `df: 0`인 질의 토큰, 곧 어휘 갭의 정의 그 자체. 블라인드 셋에서 135건 중 37건에 발동해, fan-out이 구해내는 10건 중 9건을 잡고, fan-out이 해가 되는 22건 중 오발은 1건입니다. 스킬 프로토콜도 함께 바뀝니다: 원 질문 단독으로 먼저 검색하고, `weak: true`일 때만 재작성과 함께 fan-out.

| recall@5, 블라인드 135질의 | 상시 단일 | 상시 fan-out | 조건부 (1.2.0 프로토콜) |
|---|---|---|---|
| 전체 | 0.830 | 0.741 | **0.889** |
| 어휘갭 층 | 0.517 | 0.690 | **0.724** |
| 오타 층 | 0.833 | — | **0.917** |

동결된 어휘갭 골든셋 55건에서는 **12건 회수, 회귀 0건**. 조건부가 두 고정 전략 *모두*를 이깁니다 — 게이트가 쉬운 질의의 정밀도를 지키면서, 어휘가 비는 곳에만 재작성을 씁니다.

**스니펫은 값을 하는 곳에만 컨텍스트를 얻었습니다.** 남은 evidence 미스 51건 중 23건은 정답 줄이 이미 노출 중인 줄에서 2줄 안에 있었습니다 — 문서를 못 찾은 게 아니라 표시가 정답을 잘라먹고 있던 겁니다. 이제 상위 3개 결과는 매치 라인 주변 ±2줄을 함께 담고(겹치면 연속 블록으로 병합, 최대 8픽), 그 아래 결과는 기존의 압축 형태를 유지하며, 다이어트가 필요하면 `search --compact`가 결과당 한 줄만 돌려줍니다. 블라인드 셋 evidence@10: **0.474 → 0.659** (evidence@1 0.415 → 0.607).

같은 릴리즈에서 게이트 실행이 드러낸 진짜 결정성 버그도 고쳤습니다: matched-idf 동점 처리가 Python set 순회 순서로 합산돼, 24개 해시 시드 중 1개에서 근사 동점 랭킹이 프로세스 간에 뒤집힐 수 있었습니다. 이제 질의 토큰 순서로 합산합니다 — 24개 시드 전부 동일 랭킹.
