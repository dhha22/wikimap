...
<sub>막대 = **wikimap 0.13.0 raw** · **0.14.0 fan-out** (원 질문 + 에이전트 재작성 2개, 한 번의 호출) · 선 = graphify (v1 그래프, BFS) — 정확한 수치는 아래 표</sub>

| 지표 | wikimap 0.13.0 raw | wikimap 0.14.0 fan-out | graphify |
|---|---|---|---|
...
0.13.0은 wikimap이 **모든** 검색 지표에서 graphify를 앞선 첫 버전입니다 — 같은 성격 세트에서 5배 뒤지던 v3의 역전. 향상은 전부 빌드 타임 LLM $0인 질의 시점 매칭에서 나옵니다: idf 가중 커버리지 게이트(기능어는 코퍼스 빈도로 자동 탈락, 하드코딩 스톱리스트 없음), 여러 섹션에 흩어진 매칭의 문서 단위 롤업, 긴 질의 자동 OR, 교착어 형태를 잇는 언어 비종속 term variant(`core:ui로` → `core`, `ui`).

**0.14.0은 질의 fan-out을 더합니다**: 호출 에이전트가 원 질문에 vault 어휘로 된 재작성 1~2개를 *더해* 한 번의 `search` 호출로 넘기면, 랭킹이 RRF(균등 가중)로 융합됩니다. 원 질문은 항상 투표자라 재작성이 원 질문을 대체하지 않고 회수만 더합니다 — 하방은 캡되고, 질문의 표현이 정답 문서 어휘와 전혀 겹치지 않던 하드 미스가 전부 회수 가능해집니다: top-40 미스 14건 → 0건. 재작성은 질의 시점 에이전트의 몫이라(코어에 LLM 없음, 같은 어시스턴트 턴에 실려 추가 API 왕복 0) 3개 표현 융합의 비용은 0.15.0 기준 단일 질의 대비 **~+0.1초** — 프로세스 기동·인덱스 로드·문서 haystack이 모두 공통 비용입니다. recall@1/@3 소폭 하락은 RRF 희석 트레이드오프이고, `search --json`이 이제 토큰별 문서 빈도(`terms: [{term, df}]`)를 보고하므로 약한 결과는 죽은(`df: 0`) 토큰만 갈아 끼워 재질의할 수 있습니다.

### 0.15.0 — 랭킹은 그대로, 지연은 절반

fan-out은 검색을 좋게 만든 대신 느리게 만들었습니다: 3개 표현 융합은 곧 3회 전수 스캔이었고, 단일 질의도 ~0.30초까지 밀려 있었습니다. 0.15.0은 순수한 **랭킹 불변(ranking-invariant)** 최적화입니다 — 프로파일 결과 질의 시간의 ~70%가 섹션별 variant 스캔, ~19%가 idf용 문서 haystack 재구성이었으므로, 해법은 재채점이 아니라 캐싱입니다: 불리언 히트 판정에서 피지배 variant 제거, idf 패스에서 얻은 문서 단위 term 프리필터, 문서별 title/alias/path 매치 캐싱, 프로세스당 haystack 1회 빌드(fan-out이 재사용).

| | 0.14.0 | 0.15.0 | 랭킹 변화 |
|---|---|---|---|
| 단일 질의 (median) | 0.30초 | **0.15초** | **74건 중 0건** |
| 3표현 fan-out (median) | 0.66초 | **0.26초** | **74건 중 0건** |

불변성이 핵심이고, 주장이 아니라 검증된 사실입니다: 두 경로 148개 랭킹이 0.14.0과 바이트 단위로 동일하고, 모든 recall/MRR이 소수점 셋째 자리까지 그대로이며, 테스트 104건이 통과합니다. **랭킹을 하나라도 움직인 속도 개선은 개선이 아니라 회귀입니다** — 그래서 v5 recall@5는 0.873 그대로이고, 위의 정확도 서술은 한 글자도 고칠 필요가 없습니다.
...
`search`, `links`, `path`, `suggest`, `notes`, `edges`, `semsearch`는 모두 **`--json`**을 받습니다 — 에이전트·스크립트용 구조화 출력, 사람용 출력을 regex로 긁을 필요 없음. `search --json`은 결과가 비었거나 partial이거나 최고 스코어가 낮을 때 `weak: true`를 실어, 에이전트가 질의를 문서 어휘로 재작성하거나 `search --hybrid` / `semsearch`로 온디맨드 임베딩을 섞을 신호를 주며, 토큰별 문서 빈도 `terms: [{term, df}]`도 함께 보고합니다 — 재작성 때 죽은(`df: 0`) 토큰만 정확히 갈아 끼우고 맞은 토큰은 유지하면 됩니다. 스키마는 안정적이며 테스트 스위트가 커버합니다.
